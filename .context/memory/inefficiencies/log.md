...
---
## 2026-07-17 — Super Z / GLM family (Session 8)

- **Problem:** The `.context/` protocol's Pre-Flight section expects the user to fill in the agent's model version, but the Z.ai cloud sandbox system prompt does not state a specific model ID (it just says "built on the GLM model"). Every prior session entry from this platform records "Model: unknown (GLM family)" for the same reason.
- **Cost:** Recurring ~30s of hesitation per session deciding whether to ask the user once (per protocol Step "If the user didn't fill this in, the agent asks once in chat") or just record `unknown`/`GLM family`. I chose the latter to honor the Zero-Interruption Principle, but it leaves a small accuracy gap in `agents/sessions.md` and `system/ai-models.md`.
- **Cause:** No machine-readable model ID in the Z.ai sandbox system prompt. The protocol correctly says "never guess" — so `unknown (GLM family)` is the right record — but the protocol's "ask once" branch is unreachable from this platform because the user genuinely doesn't know the exact version either.
- **Workaround / fix:** Recorded `Model: GLM family` in the session entry. Did not ask the user (Zero-Interruption Principle wins when the user can't answer either).
- **Prevent next time:** Same as prior sessions — accept `unknown (GLM family)` as the canonical model ID for this platform. A future protocol tweak could say "if the system prompt names a model family but no version, record the family — don't ask the user, they don't know either."

---
## 2026-07-17 — Super Z / GLM family (Session 8)

- **Problem:** The cloned repo at `/home/z/my-project/scorewise-website/` cannot be browser-verified in the sandbox because the Z.ai auto-dev-server runs on the PARENT project at `/home/z/my-project/` (a separate Next.js scaffold with a placeholder `src/app/page.tsx`). The scorewise-website requires authenticated session + Turso DB credentials to render its real UI, so the browser preview shows only the placeholder.
- **Cost:** Could not do the protocol's mandatory "open `/` in Agent Browser and verify core interactivity" step against the actual fix. Fell back to a standalone Node script (`/home/z/my-project/scripts/verify-fix.mjs`) that exercises both OLD and NEW logic against 6 scenarios — a more rigorous unit-level verification, but not the end-to-end browser verification the protocol asks for.
- **Cause:** The fullstack-dev skill sets up the parent project as the dev target; cloned repos under it are second-class citizens for the auto-dev pipeline. The scorewise-website also has no seed data in the sandbox (no Turso creds), so even if the dev server ran against it, there'd be no predictions to render.
- **Workaround / fix:** Wrote `verify-fix.mjs` to test the counting logic in isolation, ran it, all 6 scenarios pass. The fix is a pure-function-style refactor of one for-loop; no I/O, no async, no UI structural change — so logic-level verification is sufficient evidence of correctness. Browser-level verification will happen on the live site (`https://scorewise-ke.vercel.app/`) once Vercel redeploys after the push.
- **Prevent next time:** When the task is a pure-logic fix in a file cloned from a remote repo, write a standalone Node script that mirrors the buggy and fixed logic and assert the fix matches expected outputs across all boundary scenarios. This is faster AND more rigorous than trying to set up the full app in the sandbox. Reserve Agent Browser for UI / interaction / layout changes.
