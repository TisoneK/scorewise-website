...
---
## 2026-07-20 — Session 17 addendum (card direction CORRECTED)
- **Mistake:** Session-17 unification routed the admin Predictions tab THROUGH the user's PredictionCard, so the admin card lost its own (better) design and rendered user-style (stars + betcode footer + countdown). User: "you were supposed to keep admin's card design now you have made admin's look like the way user's was." The intent was the reverse: keep admin's design, bring the USER card up to match IT.
- **Fix:** `ab9c3f0` reverted predictions-tab.tsx + prediction-card.tsx to their pre-unification state (62829fd) → admin gets its ORIGINAL inline card back (ConfidenceBadge/RecommendationBadge bottom row, raw date + FINAL/LIVE badge header, algo strip, DID-NOT-QUALIFY/NO-BET banners, flashscore link + chevron, 2-up grid). Admin card is now UNTOUCHED going forward.
- **Then `8c87e30`:** rebuilt the USER PredictionCard to MIRROR the admin card — league-left/date+status header, teams row, O/U (single collapsed line, no strikethrough since users' reduced_* is API-stripped) + 1X2 rows, result line, and ConfidenceBadge + RecommendationBadge bottom row (replaces the old 5-star rating). Kept user-only bits admin lacks: rank pill, favorite star, kickoff countdown, betslip-code copy footer. Still hides ALL algorithm internals (no rate/above-below/per-market confidence/reduced-source). Two implementations again (admin inline + user component) but visually aligned by construction — safer than one shared component that risks re-breaking admin.
- **Rule going forward:** the ADMIN card is the canonical design; the user card follows it. Do NOT touch predictions-tab.tsx's inline card when changing the user card.
